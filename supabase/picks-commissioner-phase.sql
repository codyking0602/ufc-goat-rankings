...
-- Run after supabase/picks-event-manager-phase.sql. Safe to rerun.
...
do $$
begin
  if not exists(select 1 from pg_constraint where conname='pick_groups_owner_member_fk') then
    alter table public.pick_groups
...
set season_id=(
  select s.id from public.pick_group_seasons s
  where s.group_id=ge.group_id and s.is_active
...
set owner_member_id=(
  select gm.id from public.pick_group_members gm
  where gm.group_id=g.id and gm.is_active
  order by gm.created_at,gm.id limit 1
)
...
as $$
begin
  update public.pick_groups set owner_member_id=new.id
...
create or replace function public.picks_group_require_admin(p_group_code text,p_admin_token text)
returns uuid
language plpgsql
security definer
set search_path=public,extensions
...
begin
  ...
  if v_group_id is null then raise exception 'Only the commissioner can do that.'; end if;
  return v_group_id;
end;
$$;
...
begin
  ...
  select s.id into v_season_id from public.pick_group_seasons s
  where s.group_id=v_group.id and s.is_active order by s.starts_at desc limit 1;
  if v_season_id is null then raise exception 'Start a season before adding an event.'; end if;
  ...
  if v_room.id is null then
    ...
    values(v_group.id,p_event_id,v_room.id,v_season_id);
  ...
  return jsonb_build_object('saved',true,'group_code',v_group.code,'room_code',v_room.code,'event_id',p_event_id,'season_id',v_season_id);
end;
$$;

create or replace function public.picks_room_snapshot(p_room_code text,p_member_token text)
returns jsonb
language plpgsql
security definer
set search_path=public,extensions
...
begin
  ...
  return jsonb_build_object(
    'room',jsonb_build_object('id',v_room.id,'code',v_room.code,'name',v_room.name,'event_id',v_room.event_id,'is_result_admin',v_is_result_admin),
    'me',jsonb_build_object('id',v_me.id,'display_name',v_me.display_name),
    'my_picks',coalesce((
      select jsonb_agg(jsonb_build_object('fight_id',s.fight_id,'fighter_name',s.fighter_name,'is_underdog_lock',s.is_underdog_lock))
    ...
    'members',coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'score',(
          ...
          +(select count(*) from public.pick_selections s join public.pick_fights f on f.id=s.fight_id where s.member_id=m.id and s.is_underdog_lock and f.result_status='complete' and f.winner_name=s.fighter_name)*v_bonus_points
        ),
        'visible_picks',coalesce((
          select jsonb_agg(jsonb_build_object('fight_id',s2.fight_id,'fighter_name',s2.fighter_name,'is_underdog_lock',s2.is_underdog_lock))
...
begin
  ...
  with season_events as (
    ...
    select
      ...
      count(sel.fight_id)::integer picks_made,
...
create or replace function public.picks_commissioner_snapshot(p_group_code text,p_admin_token text)
returns jsonb
language plpgsql
security definer
set search_path=public,extensions
...
begin
  ...
  return jsonb_build_object(
    ...
    'members',coalesce((
      select jsonb_agg(jsonb_build_object(
        'id',gm.id,'display_name',gm.display_name,'is_active',gm.is_active,'is_owner',gm.id=v_group.owner_member_id,
        'created_at',gm.created_at,'removed_at',gm.removed_at
    ...
    'events',coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'fights',coalesce((
          ...
          ) order by f.bout_order) from public.pick_fights f where f.event_id=e.id
        ),'[]'::jsonb)
...
create or replace function public.picks_commissioner_rename_group(p_group_code text,p_admin_token text,p_name text)
returns jsonb
language plpgsql
security definer
set search_path=public,extensions
...
begin
  ...
  update public.pick_groups set name=trim(p_name) where id=v_group_id;
  update public.pick_rooms set name=trim(p_name) where group_id=v_group_id;
  return jsonb_build_object('saved',true,'name',trim(p_name));
end;
$$;

create or replace function public.picks_commissioner_remove_member(p_group_code text,p_admin_token text,p_member_id uuid)
returns jsonb
language plpgsql
security definer
set search_path=public,extensions
...
begin
  select * into v_group from public.pick_groups where id=public.picks_group_require_admin(p_group_code,p_admin_token);
...
create or replace function public.picks_commissioner_begin_transfer(p_group_code text,p_admin_token text,p_target_member_id uuid)
returns jsonb
language plpgsql
security definer
set search_path=public,extensions
...
create or replace function public.picks_commissioner_transfer_status(p_group_code text,p_member_token text)
returns jsonb
language plpgsql
security definer
set search_path=public,extensions
as $$
declare v_group public.pick_groups; v_member public.pick_group_members; v_transfer public.pick_group_owner_transfers; v_pending boolean:=false;
begin
  select * into v_group from public.pick_groups where code=upper(trim(p_group_code));
  ...
  v_pending:=found;
  return jsonb_build_object('pending',v_pending,'expires_at',case when v_pending then v_transfer.expires_at else null end);
end;
$$;

create or replace function public.picks_claim_group_ownership(p_group_code text,p_member_token text,p_claim_code text)
returns jsonb
language plpgsql
security definer
set search_path=public,extensions
...
declare
  ...
  v_transfer public.pick_group_owner_transfers;
...
set search_path=public,extensions
as $$
declare v_group_id uuid; v_season public.pick_group_seasons;
begin
  v_group_id:=public.picks_group_require_admin(p_group_code,p_admin_token);
  select * into v_season from public.pick_group_seasons where group_id=v_group_id and is_active;
  if not found then raise exception 'Active season not found.'; end if;
  if exists(
    select 1 from public.pick_selections sel
...
begin
  ...
  if p_correct_points not between 1 and 5 or p_underdog_bonus not between 0 and 5 then raise exception 'Scoring values are outside the allowed range.'; end if;
  update public.pick_group_seasons set is_active=false,ended_at=now() where group_id=v_group_id and is_active;
  insert into public.pick_group_seasons(group_id,name,correct_points,underdog_bonus,is_active)
  values(v_group_id,trim(p_name),p_correct_points,p_underdog_bonus,true)
  returning * into v_season;
  update public.pick_groups set active_event_id=null where id=v_group_id;
  return jsonb_build_object('started',true,'season_id',v_season.id,'name',v_season.name);
end;
$$;

create or replace function public.picks_commissioner_reopen_event(p_group_code text,p_admin_token text,p_event_id text)
returns jsonb
language plpgsql
security definer
set search_path=public,extensions
as $$
declare v_group_id uuid; v_event_status text; v_room_code text; v_season_active boolean:=false;
begin
  v_group_id:=public.picks_group_require_admin(p_group_code,p_admin_token);
  select e.status,r.code,s.is_active
  into v_event_status,v_room_code,v_season_active
  from public.pick_group_events ge
  join public.pick_events e on e.id=ge.event_id
  join public.pick_rooms r on r.id=ge.room_id
  join public.pick_group_seasons s on s.id=ge.season_id
  where ge.group_id=v_group_id and ge.event_id=p_event_id;
  if not found then raise exception 'Event not found in this group.'; end if;
  if v_event_status<>'complete' then raise exception 'Only a completed event can be reopened.'; end if;
  update public.pick_events set status='live' where id=p_event_id;
  if v_season_active then update public.pick_groups set active_event_id=p_event_id where id=v_group_id; end if;
  return jsonb_build_object('reopened',true,'event_id',p_event_id,'room_code',v_room_code);
end;
$$;
...
begin
  v_group_id:=public.picks_group_require_admin(p_group_code,p_admin_token);
  if not exists(
    select 1 from public.pick_fights f
    join public.pick_group_events ge on ge.event_id=f.event_id
    where f.id=p_fight_id and ge.group_id=v_group_id
  ) then raise exception 'Fight not found in this group.'; end if;
  ...
  where id=p_fight_id;
  return jsonb_build_object('saved',true,'fight_id',p_fight_id);
end;
$$;

grant execute on function public.picks_group_require_admin(text,text) to anon,authenticated;
