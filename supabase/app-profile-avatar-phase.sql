-- Universal UFC App member profiles, uploaded photos, and fighter avatars.
-- Run after supabase/app-canonical-group-goat26-phase.sql. Safe to rerun.
...
create or replace function public.app_profile_set_avatar(
  p_member_token text,
  p_fighter_avatar_slug text
)
returns jsonb
language plpgsql
security definer
...
declare
  v_group public.pick_groups;
  v_member public.pick_group_members;
  v_slug text:=nullif(lower(trim(coalesce(p_fighter_avatar_slug,''))), '');
begin
  if v_slug is not null and (
    char_length(v_slug) not between 2 and 80
    or v_slug !~ '^[a-z0-9]+(-[a-z0-9]+)*$'
  ) then
    return jsonb_build_object('ok',false,'error','Choose a valid UFC fighter avatar.');
  end if;
  ...
  set fighter_avatar_slug=v_slug,
...
grant execute on function public.app_profile_set_avatar(text,text) to anon,authenticated;
