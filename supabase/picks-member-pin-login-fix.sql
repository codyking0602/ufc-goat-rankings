-- Picks member PIN sign-in reliability patch.
-- Run after supabase/picks-member-pin-phase.sql. Safe to rerun.
-- Makes group-code entry tolerant of common character mistakes and returns useful errors.
...
declare
  ...
  v_group_code text:=upper(regexp_replace(coalesce(p_group_code,''),'[^A-Za-z0-9]','','g'));
begin
  -- Group codes use hexadecimal characters, so these letters can only be mistyped numbers.
  v_group_code:=translate(v_group_code,'OILSZGQ','0115260');
  ...
  if not found then raise exception 'Group code was not found.'; end if;
...
begin
  ...
  if not found then raise exception 'Commissioner access did not match this group.'; end if;
...
begin
  -- Codes are generated from 0-9 and A-F only. Normalize common visual substitutions.
  v_group_code:=translate(v_group_code,'OILSZGQ','0115260');

  if char_length(v_group_code)<>6 then
    return jsonb_build_object('ok',false,'error','Group code must be 6 characters.');
  end if;
  ...
  if not found then
    return jsonb_build_object('ok',false,'error','That group code was not found. Check 0 versus O.');
  end if;
