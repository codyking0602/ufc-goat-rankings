...
window.UFC_MODULE_VERSIONS={
  scoringPipeline:"20260710e-prime-complete",
  ...
  primeDominanceAuditBatchSeven:"20260710c-prime-final-batch",
...
(function(){
  ...
  const VERSION='deterministic-scoring-pipeline-20260710e-prime-complete';
  const state={
    version:VERSION,
  ...
  function cache(path,name){return `${path}?v=${name}`;}
  function record(label){state.sequence.push(label);}
  ...
  function renderOnce(){
    if(typeof window.refresh==='function'){
      try{window.refresh();}catch(e){state.renderError=String(e?.message||e);}
    }
  }
