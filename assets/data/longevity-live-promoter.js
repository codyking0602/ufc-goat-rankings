  ...
  function apply(){
    ...
    try{
      ...
      rows().forEach(row=>{
        ...
        if(!shadow){if(!seenMissing.has(fighterKey))missing.push(fighter);seenMissing.add(fighterKey);return;}
        ...
        row.activeEliteYears=round2(shadow.activeEliteYears);
        row.longevitySource='fighter-era-ledger';
        row.longevityPromoterVersion=VERSION;
        row.longevityAudit={
          version:VERSION,source:'fighter-era-ledgers via longevity-shadow-scorer',shadowVersion:SHADOW.version||null,
          eraLedgerVersion:SHADOW.sourceEraLedgerVersion||window.UFC_FIGHTER_ERA_LEDGERS?.version||null,
          formula:SHADOW.formula||'score = min(30, countedEliteMonths / 144 × 30)',raw30,weighted10,legacyScore:legacy,
          legacyDelta:round2(raw30-legacy),shadowRank:shadow.shadowRank||null,window:shadow.window||null,rawWindowLabel:shadow.rawWindowLabel||null,
          activeEliteYears:shadow.activeEliteYears,gapAdjustedMonths:shadow.gapAdjustedMonths,gapCapMonths:shadow.gapCapMonths,
          statusMultiplier:shadow.statusMultiplier,divisionMultiplier:shadow.divisionMultiplier,countedEliteMonths:shadow.countedEliteMonths,
          ceilingApplied:!!shadow.ceilingApplied,endType:shadow.endType||null,endReason:shadow.endReason||'',notes:shadow.notes||[],appliedAt:new Date().toISOString()
        };
        const profile=profileFor(fighter);
        if(profile){profile.activeEliteYears=round2(shadow.activeEliteYears);profile.longevity=raw30;profile.longevityAudit={...row.longevityAudit};}
      ...
      if(DATA.meta){
        ...
        DATA.meta.longevityLivePromotion={appliedCount:unique.length,missing:[...missing],shadowVersion:SHADOW.version||null,eraLedgerVersion:SHADOW.sourceEraLedgerVersion||window.UFC_FIGHTER_ERA_LEDGERS?.version||null,maxMonths:SHADOW.maxMonths||144,categoryOnly:true,appliedAt:new Date().toISOString()};
      }
