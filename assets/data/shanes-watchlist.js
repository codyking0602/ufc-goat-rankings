(function(){
  ...
  const VERSION='shanes-watchlist-20260719c-fatima-links-photos';
  ...
  const photoPath=fighter=>`assets/fighters/${encodeURIComponent(text(fighter.photoSlug||fighter.id))}-thumb.webp`;
  ...
  function fighterCard(fighter,index){
    ...
    return `<article class="home-shane-card${index===0?' is-latest':''}" data-shane-watch-fighter="${esc(fighter.id)}">
      ...
      <div class="home-shane-identity"><span class="home-shane-avatar"><img data-shane-photo src="${esc(photoPath(fighter))}" alt="${esc(fighter.name)} headshot" loading="lazy"><b aria-hidden="true">${esc(initials(fighter.name))}</b></span><div><h4>${esc(display(fighter.name))}</h4>${fighter.nickname?`<p>“${esc(fighter.nickname)}”</p>`:''}</div></div>
  ...
  function wirePhotos(section){
    section.querySelectorAll('[data-shane-photo]').forEach(image=>{
      ...
      if(!avatar||image.dataset.photoWired==='true')return;
      image.dataset.photoWired='true';
      const show=()=>avatar.classList.add('has-photo');
      const hide=()=>{avatar.classList.remove('has-photo');image.hidden=true;};
  ...
  function injectStyles(){
    ...
    style.textContent=`
      ...
      .home-shane-identity{display:grid;grid-template-columns:58px minmax(0,1fr);gap:12px;align-items:center}.home-shane-avatar{position:relative;width:58px;height:58px;display:grid;place-items:center;overflow:hidden;border:1px solid rgba(var(--shane-glow),.42);border-radius:15px;background:linear-gradient(145deg,rgba(var(--shane-glow),.28),#172033 68%);color:#fff;font-size:14px;font-weight:950}.home-shane-avatar img{display:none;position:absolute;inset:0;width:100%;height:100%;object-fit:cover}.home-shane-avatar.has-photo img{display:block}.home-shane-avatar.has-photo b{display:none}.home-shane-avatar b{font:950 14px/1 system-ui}.home-shane-identity h4{margin:0;color:#fff;font-size:17px;font-weight:950;line-height:1.15}.home-shane-identity p{margin:3px 0 0;color:var(--shane-accent);font-size:11px;font-weight:850}.home-shane-meta{margin-top:11px;color:#b5c0d0;font-size:11px;font-weight:750}
