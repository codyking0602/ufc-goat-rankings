// Penalty score corrections for active 37 fighters.
// Applies Cody-approved locked loss-penalty rules after Longevity corrections and before score-derived OVR.
(function(){
  const VERSION = 'penalty-score-corrections-20260705a';
  ...
  const corrections = {
    ...
    'Frankie Edgar': { penalty: -9.75, notes: 'Benson, Aldo, Ortega, and Holloway count; late bantamweight collapse is post-prime.' },
  ...
  function patchRow(row){
    ...
    row.penaltyAudit = {
      ...
      rules: RULES,
      notes: c.notes,
      source: 'Penalty active-37 worksheet',
      version: VERSION
    };
  }
  ...
  (DATA.fighters || []).forEach(profile => {
    ...
    if(penaltyByFighter.has(profile.fighter)) {
      profile.penalty = penaltyByFighter.get(profile.fighter);
      profile.lossPenalty = penaltyByFighter.get(profile.fighter);
    }
  });
  ...
  if(typeof DISPLAY_OVERRIDES !== 'undefined'){
    rankByFighter.forEach((rank,fighter) => {
      ...
      if(DISPLAY_OVERRIDES[fighter].categories?.penalty){
        delete DISPLAY_OVERRIDES[fighter].categories.penalty;
      }
    });
  }

  window.UFC_PENALTY_SCORE_CORRECTIONS = {
    version: VERSION,
