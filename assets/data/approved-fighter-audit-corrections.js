...
// Keeps canonical era, opponent-quality, and Prime Dominance corrections in one permanent pass.
(function(){
  'use strict';
  ...
  if(tito){
    ...
    if(ERA.canonicalWindowLock){
      ERA.canonicalWindowLock.changedWindowFighters=Array.from(new Set([...(ERA.canonicalWindowLock.changedWindowFighters||[]),'Tito Ortiz']));
  ...
  function applyPrimeDominanceCorrection(){
    if(window.__UFC_USMAN_STIPE_DC_PRIME_CORRECTION_APPLIED)return window.UFC_APPROVED_PRIME_DOMINANCE_CORRECTIONS||{applied:true,idempotent:true};
    ...
    Object.entries(corrections).forEach(([fighter,input])=>{
      ...
      const finishRate=input.fights?Number(entry.primeFinishes||0)/input.fights:0;
      const pressureScore=finishScore(finishRate,ledgers.finishScale);
      const total=round2(recordScore+Number(entry.roundControlScore||0)+pressureScore+Number(entry.eliteStakesScore||0));
      Object.assign(entry,{primeRecord:input.record,primeWins:input.wins,primeLosses:input.losses,primeFights:input.fights,primeRecordPct:round2(pct),primeRecordScore:recordScore,primeFinishRate:round2(finishRate*100),finishPressureScore:pressureScore,total,status:'locked',approvedAuditCorrection:true,approvedAuditReason:input.reason,version:VERSION});
      ...
      DATA.primeRecords[fighter]={...(DATA.primeRecords[fighter]||{}),record:input.record,source:'Cody-approved canonical fight-window recount',sourceVersion:VERSION,approvedReason:input.reason};
      patchDisplay(fighter,{snapshotStats:{primeRecord:input.record,primeDominanceShadow:total,primeFinishRate:`${round2(finishRate*100)}%`,primeRecordContext:input.reason}});
      results.push({fighter,primeRecord:input.record,primeDominance:total,primeRecordScore:recordScore,finishPressureScore:pressureScore});
    });
  ...
  const API={version:VERSION,applied:true,eraCorrections:['Tito Ortiz'],primeRecordCorrections:Object.keys(PRIME_RECORD_CORRECTIONS),opponentQualityCorrections:['Kayla Harrison','Kamaru Usman','Stipe Miocic','Daniel Cormier'],primeDominanceCorrections:['Kamaru Usman','Stipe Miocic','Daniel Cormier'],masvidalCreditsChanged:false,applyOpponentQualityCorrection,applyPrimeDominanceCorrection,appliedAt:new Date().toISOString()};
  window.UFC_APPROVED_FIGHTER_AUDIT_CORRECTIONS=API;
  DATA.meta=DATA.meta||{};
  DATA.meta.approvedFighterAuditCorrections={version:VERSION,eraCorrections:API.eraCorrections,primeRecordCorrections:API.primeRecordCorrections,opponentQualityCorrections:API.opponentQualityCorrections,primeDominanceCorrections:API.primeDominanceCorrections,masvidalCreditsChanged:false,appliedAt:API.appliedAt};
