// Phase 1 batch eight: ten complete audited UFC-only fight ledgers.
// Evidence only. Never mutates live scores, totals, rank, OVR, snapshots, profiles, or Compare Mode.
(function(){
'use strict';
const VERSION='canonical-fighter-facts-batch-eight-20260713a-ten-fighters-176';
const API=window.UFC_CANONICAL_FIGHTER_FACTS;
const fail=(error,details=[])=>{window.UFC_CANONICAL_FIGHTER_FACTS_BATCH_EIGHT={version:VERSION,applied:false,error:String(error||'Unknown batch error.'),details,mutatesRankingData:false};throw new Error(`[${VERSION}] ${error}`);};
...
const DIVISION={Y:'Flyweight',B:'Bantamweight',F:'Featherweight',G:'Lightweight',W:'Welterweight',M:'Middleweight',L:'Light Heavyweight',H:'Heavyweight',C:'Catchweight',O:'Openweight'};
const slug=value=>String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[’‘`´]/g,"'").toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'');
const DATA=window.UFC_CANONICAL_FIGHTER_FACTS_BATCH_EIGHT_DATA||[];
...
const expected=['Khamzat Chimaev','Leon Edwards',"Sean O'Malley",'Sean Strickland','Michael Bisping','Dan Henderson','Chael Sonnen','Robbie Lawler','Frank Shamrock','Royce Gracie'];
if(JSON.stringify(records.map(record=>record.fighter))!==JSON.stringify(expected))fail('Batch-eight fighter order or identity mismatch.',records.map(record=>record.fighter));
...
const invalid=records.map(record=>({fighter:record.fighter,...API.validate(record)})).filter(row=>!row.valid);if(invalid.length)fail('One or more batch-eight records failed validation.',invalid);
...
if(fighters.length!==10||fightCount!==176)fail(`Expected 10 fighters and 176 fights; received ${fighters.length} fighters and ${fightCount} fights.`);
window.UFC_CANONICAL_FIGHTER_FACTS_BATCH_EIGHT={version:VERSION,applied:true,recordCount:fighters.length,fightCount,fighters,derived:Object.fromEntries(fighters.map(fighter=>[fighter,API.deriveFor(fighter)])),mutatesRankingData:false};
document.documentElement.setAttribute('data-canonical-fighter-facts-batch-eight',VERSION);
})();
