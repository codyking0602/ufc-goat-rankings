(function(root){
  ...
  const VERSION='blind-rank-category-architecture-20260717a-phase-one';
  ...
  const CATEGORIES=[
    {
      id:'ufc-career',
      ratingPath:'career.overall',
      name:'UFC Career',
      group:'Career',
      status:'launch',
      definition:'UFC-only greatness across championship accomplishment, opponent quality, prime dominance, active elite longevity, and loss context.',
      criteria:['Championship','Opponent Quality','Prime Dominance','Longevity','Penalty Context']
    },
    {
      id:'striking',
      ratingPath:'striking',
      ...
      group:'Skill',
      status:'launch',
      definition:'UFC striking effectiveness, craft, defense, adaptability, and performance against strong opposition.',
      criteria:['Effectiveness','Craft','Defense','Adaptability','Elite-Level Proof']
    },
    {
      id:'wrestling-grappling',
      ratingPath:'grappling',
      name:'Wrestling & Grappling',
      group:'Skill',
      status:'launch',
      definition:'UFC wrestling, submissions, positional control, transitions, defensive grappling, and ability to impose the phase.',
      criteria:['Wrestling Control','Submissions','Position','Transitions','Defensive Grappling']
    },
    {
      id:'hardest-at-peak',
      ratingPath:'peak',
      name:'Hardest to Beat at Peak',
      group:'Debate',
      status:'launch',
      definition:'How difficult the fighter was to defeat during their best UFC stretch, independent of total career length.',
      criteria:['Prime Control','Round Separation','Durability','Adaptability','Opponent Level']
    },
    {
      id:'best-finisher',
      ratingPath:'finishing',
      name:'Best Finisher',
      group:'Debate',
      status:'launch',
      definition:'UFC finishing threat using rate, volume, opponent quality, title stakes, variety, and repeatability.',
      criteria:['Finish Rate','Finish Volume','Elite Finishes','Title Stakes','Variety']
    },
    {
      id:'most-complete',
      ratingPath:'complete',
      name:'Most Complete Fighter',
      group:'Debate',
      status:'launch',
      definition:'Breadth and balance across striking, wrestling, grappling, defense, cardio, and adaptability.',
      criteria:['Striking','Wrestling','Grappling','Defense','Cardio','Adaptability']
    },
    {
      id:'action-fighter',
      ratingPath:'action',
      ...
      group:'Entertainment',
      status:'launch',
      definition:'Reliable UFC entertainment through pace, violence, volatility, memorable fights, and willingness to engage.',
      criteria:['Pace','Damage','Volatility','Memorable Fights','Reliability']
    },
    {
      id:'ufc-star-power',
      ratingPath:'starPower',
      name:'UFC Star Power',
      group:'Entertainment',
      status:'planned',
      definition:'UFC drawing power, mainstream recognition, cultural footprint, and sustained relevance.',
      criteria:['Drawing Power','Mainstream Reach','Cultural Impact','UFC Relevance']
    }
  ];
  ...
  const LINEUP_RULES={
    ...
    fallback:{
      primary:'exact-tier',
      secondary:'adjacent-tier',
      maximumTierDistance:1,
      emergency:'any-rated-fighter-with-audit-warning'
    },
    repeatProtection:{
      historyScope:'per-pack',
      recentRevealWindow:15,
      relaxWindows:[15,10,5,0]
    }
  };
  ...
  const RATING_CONTRACT={
    ...
    optionalPaths:[
      'career.divisions.lightweight',
      'career.divisions.welterweight',
      'career.divisions.heavyweight',
      'starPower'
    ],
    recordShape:{
      ...
      ratings:{
        career:{overall:'0-100',divisions:{lightweight:'0-100',welterweight:'0-100',heavyweight:'0-100'}},
        striking:'0-100',
        grappling:'0-100',
        peak:'0-100',
        finishing:'0-100',
  ...
  function validScore(value){
    return Number.isFinite(Number(value))&&Number(value)>=RATING_CONTRACT.scoreMinimum&&Number(value)<=RATING_CONTRACT.scoreMaximum;
  }
  ...
  function roleFor(id){
    return LINEUP_ROLES.find(role=>role.id===id)||null;
  }

  function ratingForPack(record,packId){
    const pack=packFor(packId);
    if(!pack)return null;
    for(const path of pack.ratingPaths){
      const value=getPath(record?.ratings,path);
      if(validScore(value))return {score:Number(value),tier:tierForScore(value),path,fallback:path!==pack.ratingPaths[0]};
    }
    return null;
  }
  ...
  function validateRatingRecord(record){
    const errors=[];
    const fighterId=String(record?.fighterId||'').trim();
    const fighterName=String(record?.fighterName||'').trim();
    const ineligible=new Set(Array.isArray(record?.ineligible)?record.ineligible:[]);
    if(!fighterId)errors.push('fighterId is required.');
    if(!fighterName)errors.push('fighterName is required.');
    CATEGORIES.filter(category=>category.status==='launch').forEach(category=>{
      if(ineligible.has(category.id))return;
      const value=getPath(record?.ratings,category.ratingPath);
      if(!validScore(value))errors.push(`${category.ratingPath} must be rated 0-100 or ${category.id} must be explicitly ineligible.`);
    });
  ...
  function audit(){
    ...
    TIERS.forEach((tier,index)=>{
      if(tierIds.has(tier.id))errors.push(`Duplicate tier id: ${tier.id}`);
      tierIds.add(tier.id);
      if(!Number.isFinite(tier.minScore)||tier.minScore<0||tier.minScore>100)errors.push(`${tier.id} has an invalid minimum score.`);
      if(index&&tier.minScore>=TIERS[index-1].minScore)errors.push('Tier minimums must descend from Elite to Bad.');
    });
    for(let score=0;score<=100;score+=0.25){
      if(!tierForScore(score))errors.push(`No tier covers score ${score}.`);
    }

    CATEGORIES.forEach(category=>{
      if(categoryIds.has(category.id))errors.push(`Duplicate category id: ${category.id}`);
      categoryIds.add(category.id);
      if(!category.ratingPath)errors.push(`${category.id} needs a rating path.`);
      if(!['launch','planned'].includes(category.status))warnings.push(`${category.id} is marked ${category.status}.`);
    });

    PACKS.forEach(pack=>{
      if(packIds.has(pack.id))errors.push(`Duplicate pack id: ${pack.id}`);
      packIds.add(pack.id);
      if(!categoryIds.has(pack.categoryId))errors.push(`${pack.id} references unknown category ${pack.categoryId}.`);
      if(!Array.isArray(pack.ratingPaths)||!pack.ratingPaths.length)errors.push(`${pack.id} needs at least one rating path.`);
    });

    LINEUP_ROLES.forEach(role=>{
      if(roleIds.has(role.id))errors.push(`Duplicate lineup role id: ${role.id}`);
      roleIds.add(role.id);
      const entries=Object.entries(role.weights||{});
      const total=entries.reduce((sum,[tier,weight])=>{
        if(!tierIds.has(tier))errors.push(`${role.id} references unknown tier ${tier}.`);
        if(!Number.isFinite(weight)||weight<0)errors.push(`${role.id} has invalid weight for ${tier}.`);
        return sum+(Number(weight)||0);
      },0);
      if(Math.abs(total-1)>EPSILON)errors.push(`${role.id} weights total ${total}, not 1.`);
    });

    if(LINEUP_ROLES.length!==LINEUP_RULES.lineupSize)errors.push('The number of lineup roles must equal the lineup size.');
    const wildcard=roleFor('wildcard');
    if(Math.abs((wildcard?.weights?.bad||0)-LINEUP_RULES.badGameTargetRate)>EPSILON)errors.push('Wildcard Bad weight must match the target Bad-game rate.');
    ...
    return {
      ...
      counts:{
        ...
        launchCategories:CATEGORIES.filter(category=>category.status==='launch').length,
        packs:PACKS.length,
        lineupRoles:LINEUP_ROLES.length
      }
    };
  }
  ...
  const api=deepFreeze({
    ...
    packs:PACKS,
    lineupRoles:LINEUP_ROLES,
    lineupRules:LINEUP_RULES,
    ratingContract:RATING_CONTRACT,
    tierForScore,
    categoryFor,
    packFor,
    roleFor,
