...
(function(){
  ...
  const VERSION='loss-context-hybrid-shadow-20260711a-phase-one';
  ...
  if(!DATA||!ERA?.ledgers||!ADAPTER){
    window.UFC_LOSS_CONTEXT_HYBRID_SHADOW={version:VERSION,applied:false,error:[!DATA?'Missing RANKING_DATA':null,!ERA?.ledgers?'Missing Fighter Era ledgers':null,!ADAPTER?'Missing canonical Loss Context adapter':null].filter(Boolean).join('; '),mutatesScores:false,mutatesPenalty:false};
    return;
  }
