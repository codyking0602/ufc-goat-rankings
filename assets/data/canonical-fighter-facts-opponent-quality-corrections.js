...
(function(){
  ...
  const VERSION='canonical-fighter-facts-opponent-quality-corrections-20260714a';
  ...
  if(!alreadyPresent){
    ...
    applied.push({fighter:'B.J. Penn',fightId,opponent:'Duane Ludwig',action:'added-official-ufc-win'});
  }else{
    applied.push({fighter:'B.J. Penn',fightId:alreadyPresent.id,opponent:'Duane Ludwig',action:'already-present'});
  }
  ...
  const report={version:VERSION,applied:true,correctionCount:applied.filter(row=>row.action==='added-official-ufc-win').length,applied,rejectedFalseFactAdditions,canonicalFighterCount:API.count(),canonicalFightCount:audit.results.reduce((sum,row)=>sum+(API.get(row.fighter)?.fights?.length||0),0),mutatesRankingData:false};
  window.UFC_CANONICAL_FIGHTER_FACTS_OPPONENT_QUALITY_CORRECTIONS=report;
