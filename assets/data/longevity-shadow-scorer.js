...
(function(){
  const VERSION='longevity-shadow-scorer-20260709b-ledger-driven';
  ...
  function hasLongevityInputs(ledger){
    return ledger&&ledger.longevity&&Number.isFinite(Number(ledger.longevity.gapAdjustedMonths));
  }
  ...
  function buildRow(fighter){
    ...
    return {
      ...
      activeEliteYears:round2(longevity.activeEliteYears),
      gapAdjustedMonths:round2(gapAdjustedMonths),
      gapCapMonths:Number(longevity.gapCapMonths||era.rules?.gapCapMonthsDefault||18),
      ...
      notes:[
        longevity.adjustmentNote||'',
        longevity.note||'',
        ledger.notes||''
      ].filter(Boolean),
      source:'fighter-era-ledgers longevity inputs',
      mutatesScores:false
    };
  }
