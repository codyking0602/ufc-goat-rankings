// Adds Merab's recent Petr Yan rematch loss to Loss Context.
(function(){
  const VERSION='merab-yan-loss-context-20260707a';
  const TARGET='Merab Dvalishvili';
  const YAN_LOSS={opponent:'Petr Yan',phase:'prime',opponentTier:'championTop5',finished:false,finishTreatment:'none',counted:true,notes:'Title-level rematch loss by decision.'};
  const CAP=-10;
  ...
  function apply(){
    const f=rows().find(x=>x?.fighter===TARGET);
    if(!f)return false;
    f.losses=Array.isArray(f.losses)?f.losses.slice():[];
    if(!f.losses.some(x=>String(x?.opponent||'').toLowerCase()==='petr yan'))f.losses.push({...YAN_LOSS});
    ...
    f.lossContextFormulaDriven=true; f.lossContextFormulaVersion=VERSION; f.lossContextFormulaWrite={previous,positiveScore:pos,targetPenalty:target,rawPenalty:raw,source:'Merab vs Petr Yan rematch update'};
