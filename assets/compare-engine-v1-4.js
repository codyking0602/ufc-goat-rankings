(function () {
  ...
  function ledgerEntry(a, b) {
    ...
    if (!item) return null;
    return { ...item, summary: clean(item.summary) };
  }
  ...
  function winnerParagraph(winner, loser, key) {
    ...
    const body = `${opener} the stronger title run, the better elite-years case, or both. ${stat} ${prime ? `${prime}.` : ""} ${sig}`;
    return clean(body);
  }
  ...
  function summaryParagraph(winner, loser, angle, h2h, key) {
    ...
    return clean(`${loser.fighter} has the counterargument. ${winner.fighter} has the stronger title value, active elite years, and complete resume. ${bridge}`);
  }
  ...
  function angleCopy(winner, loser, angle, type, h2h, key) {
    const paragraphs = [];
    if (angle === "directFight" && h2h) paragraphs.push(clean(h2h.summary));
    if (angle === "scopeContext") paragraphs.push(clean(field(loser, "scope", field(winner, "scope", "Some of the historical case sits outside this comparison's UFC boundary, so it needs context."))));
  ...
  function bootCompareEngineV14() {
    const previousRender = window.renderCompare;
    window.renderCompare = function compareEngineV14Render() {
      if (typeof previousRender === "function") previousRender();
      setTimeout(renderV14IfGeneric, 0);
    };
    try { renderCompare = window.renderCompare; } catch (e) {}

    ["fighterA", "fighterB"].forEach(id => {
      const select = document.getElementById(id);
      if (select && !select.dataset.compareEngineV14Bound) {
        select.dataset.compareEngineV14Bound = "true";
        select.addEventListener("change", () => setTimeout(renderV14IfGeneric, 0));
      }
    });
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", bootCompareEngineV14);
  } else {
    bootCompareEngineV14();
  }
})();
