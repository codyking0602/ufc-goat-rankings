...
(function(){
  const VERSION='championship-resume-live-20260710a-category-only';
  ...
  function ensureOverride(name){
    ...
    override.snapshotStats=override.snapshotStats||{};
    if(override.categories?.championship)delete override.categories.championship;
    return override;
  }
  function updateSnapshot(name,report,score){
    const o=ensureOverride(name);if(!o)return;
    o.snapshotStats.titleFightWins=report.titleFightWins;
    o.snapshotStats.adjustedTitleWins=r(report.adjustedTitleCredit);
    o.snapshotStats.championshipScore=score;
    const titleWinsValue=String(report.titleFightWins);
    const champValue=`${score.toFixed(2)}/30`;
    const adjustedValue=n(report.adjustedTitleCredit).toFixed(2);
    const rows=Array.isArray(o.snapshot)?o.snapshot.slice():[];
    let sawTitle=false,sawChamp=false,sawAdjusted=false;
    const updated=rows.map(item=>{
      if(!Array.isArray(item)||item.length<2)return item;
      const label=String(item[0]||'');
      if(/title[-\s]*fight wins/i.test(label)){sawTitle=true;return[item[0],titleWinsValue];}
      if(/championship level|championship resume|title reign/i.test(label)){sawChamp=true;return[item[0],champValue];}
      if(/adjusted title/i.test(label)){sawAdjusted=true;return[item[0],adjustedValue];}
      return item;
    });
    if(!sawTitle)updated.push(['UFC Title-Fight Wins',titleWinsValue]);
    if(!sawChamp)updated.push(['Championship Resume',champValue]);
    if(!sawAdjusted)updated.push(['Adjusted Title Credit',adjustedValue]);
    o.snapshot=updated;
  }
  function loadProfilePolish(){
    if(document.querySelector('script[data-championship-resume-profile-card]'))return;
    const script=document.createElement('script');
    script.src='assets/js/championship-resume-profile-card.js?v=championship-resume-profile-card-20260707a';
    script.setAttribute('data-championship-resume-profile-card','true');
    document.body.appendChild(script);
  }
  function loadScriptOnce(src,attr,done){
    if(document.querySelector(`script[${attr}]`)){if(done)done();return;}
    const script=document.createElement('script');script.src=src;script.setAttribute(attr,'true');
    script.onload=()=>{if(done)done();};script.onerror=()=>{if(done)done();};document.body.appendChild(script);
  }
  function loadOpponentQualityShadow(){
    loadScriptOnce('assets/data/opponent-quality-ledgers.js?v=opponent-quality-ledgers-20260708d','data-opponent-quality-ledgers',()=>
      loadScriptOnce('assets/data/opponent-quality-ledger-batch-four.js?v=opponent-quality-ledger-batch-four-20260708a','data-opponent-quality-ledger-batch-four',()=>
        loadScriptOnce('assets/data/opponent-quality-dj-calibration.js?v=opponent-quality-dj-calibration-20260708d','data-opponent-quality-dj-calibration')));
  }
  ...
  window.UFC_CHAMPIONSHIP_RESUME_LIVE={
    ...
    leaders:{
      men:(DATA.men||[]).slice().sort((a,b)=>n(b.championship)-n(a.championship)).slice(0,10).map(row=>({fighter:row.fighter,championship:row.championship,titleFightWins:byName.get(row.fighter)?.titleFightWins,adjustedTitleCredit:byName.get(row.fighter)?.adjustedTitleCredit})),
      women:(DATA.women||[]).slice().sort((a,b)=>n(b.championship)-n(a.championship)).slice(0,10).map(row=>({fighter:row.fighter,championship:row.championship,titleFightWins:byName.get(row.fighter)?.titleFightWins,adjustedTitleCredit:byName.get(row.fighter)?.adjustedTitleCredit}))
    },
  ...
  applyFinalScore('championship-resume-category-update');
