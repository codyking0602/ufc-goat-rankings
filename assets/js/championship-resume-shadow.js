...
(function(){
  const VERSION='championship-resume-shadow-20260707c-batch-one-dynamic-benchmark';
  ...
  function titleCredit(f){return r(winsFor(f).reduce((sum,w)=>{const t=typeOf(w.titleType||w.type);const base=BASE[t]||1;const strength=clamp01(w.strength||w.multiplier||w.opponentDiscount||w.discount||1);return sum+(base*strength);},0));}
  function benchmarkCredit(list){const credits=list.map(titleCredit).filter(x=>x>0);return credits.length?Math.max.apply(null,credits):1;}
  function calculate(f,benchmark){const direct=!!directBatch(f)||Array.isArray(f.championshipWins)&&f.championshipWins.length;const wins=winsFor(f).map((w,i)=>{const t=typeOf(w.titleType||w.type);const base=BASE[t]||1;const strength=clamp01(w.strength||w.multiplier||w.opponentDiscount||w.discount||1);return{index:i+1,opponent:w.opponent||'TBD',titleType:t,baseValue:base,strength,adjustedCredit:r(base*strength),source:w.source||(direct?'directLedger':'titleBucketFallback'),notes:w.notes||''};});const credit=r(wins.reduce((sum,w)=>sum+w.adjustedCredit,0));const formulaScore=r(Math.min(MAX,Math.max(0,(credit/benchmark)*MAX)));const currentScore=r(n(f.championship||(f.scoring&&f.scoring.championship)));return{fighter:f.fighter,status:direct?'direct-ledger':'fallback-title-buckets',titleFightWins:wins.length,adjustedTitleCredit:credit,formulaScore,currentScore,delta:r(formulaScore-currentScore),wins};}
  function apply(){const list=fighters();const benchmark=benchmarkCredit(list);const report=list.map(f=>{const audit=calculate(f,benchmark);if(BATCH_ONE[f.fighter])f.championshipWins=winsFor(f);f.championshipResumeAudit=audit;f.championshipFormulaDriven=false;boards().filter(row=>row&&row.fighter===f.fighter).forEach(row=>{if(BATCH_ONE[f.fighter])row.championshipWins=winsFor(f);row.championshipResumeAudit=audit;row.championshipFormulaDriven=false;});return{fighter:audit.fighter,status:audit.status,titleFightWins:audit.titleFightWins,adjustedTitleCredit:audit.adjustedTitleCredit,currentScore:audit.currentScore,formulaScore:audit.formulaScore,delta:audit.delta};});window.UFC_CHAMPIONSHIP_RESUME_SHADOW={version:VERSION,mode:'shadow',baseValues:BASE,batchOne:Object.keys(BATCH_ONE),benchmarkCredit:benchmark,report,calculate:function(f){return calculate(f,benchmark);}};document.documentElement.setAttribute('data-championship-resume-shadow',VERSION);return report;}
