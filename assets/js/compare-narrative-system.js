...
(function(){
  const VERSION = 'compare-narrative-system-20260702g';
  ...
  const FIGHTER_VOICE = {
    ...
    'Demetrious Johnson': {
      ...
      weakness: ['The limiter is not skill. It is flyweight depth and how much weight the ranking gives his opponent ledger.']
    },
    'Anderson Silva': {
    ...
    'Islam Makhachev': {
      ...
      peak: ['Islam’s best version is control with finishing threat: patient pressure, grappling danger, improved striking, and very few easy rounds for opponents.'],
    ...
    'Khabib Nurmagomedov': {
      title: ['Khabib’s title run is shorter than the all-time volume kings, but every title-level chapter felt clean and decisive.'],
      peak: ['Khabib’s case is peak purity: no UFC losses, no real collapse, and a final stretch that looked like total separation from elite lightweights.'],
    ...
    'Alexander Volkanovski': {
      title: ['Volk’s championship case is extremely clean at featherweight: belt win, defenses, and direct separation from Holloway.'],
      peak: ['Volk’s best version was balance: footwork, adjustments, pace, defense, and the ability to win long tactical fights.'],
    ...
    'Max Holloway': {
      title: ['Holloway’s title case is strong, but the bigger argument is not belt volume; it is how much elite UFC work he kept adding.'],
    ...
    'Kamaru Usman': {
      title: ['Usman’s case starts with the champion peak: more title-fight value, more round-to-round control, and a welterweight reign that felt locked in for years.'],
      ...
      longevity: ['Usman’s elite window was not short, but it is more champion-peak than decade-long volume case.'],
    ...
    'Jose Aldo': {
      title: ['Aldo’s UFC-only title case is strong, but the full historical aura is bigger than what this ranking can score because the WEC reign sits outside the main count.'],
      peak: ['Peak Aldo was speed, takedown defense, leg kicks, composure, and a champion presence that carried into the UFC era.'],
      opponentQuality: ['Aldo’s UFC list has real depth across featherweight and bantamweight, even with the WEC chapter treated as context.'],
      longevity: ['Aldo’s best UFC-only argument is longevity: he stayed relevant through multiple generations and even rebuilt value at bantamweight.'],
    ...
    'Daniel Cormier': {
      title: ['Cormier’s case is two-division title value: light heavyweight gold, heavyweight gold, and a dense elite UFC window.'],
      peak: ['DC’s best version was pressure wrestling, clinch control, durability, and the ability to compete up at heavyweight without looking out of place.'],
      opponentQuality: ['Cormier’s UFC ledger is compact but high-end, with heavyweight and light heavyweight value packed into a shorter window.'],
      longevity: ['DC’s UFC run is dense more than long. He arrived late, but almost every chapter mattered.'],
    ...
    'Stipe Miocic': {
      ...
      longevity: ['Stipe’s case is not just one reign; it is heavyweight staying power across multiple dangerous styles.'],
    ...
    'Alex Pereira': {
      ...
      peak: ['Pereira’s peak is consequence. Every exchange feels dangerous because one clean shot can rewrite the whole fight.'],
      opponentQuality: ['Adesanya, Prochazka twice, Hill, Blachowicz, Strickland, and Rountree make Pereira’s short UFC ledger loud.'],
      longevity: ['The volume is still building, but the title impact already punches above the length of the run.'],
      weakness: ['The question is length. Pereira’s high end is massive, but the UFC sample is shorter than the deepest all-time cases.']
    },
    'Israel Adesanya': {
      title: ['Adesanya’s case is middleweight title volume: repeated defenses, two Whittaker wins, and years as the division’s standard.'],
    ...
    'Amanda Nunes': {
      ...
      opponentQuality: ['Shevchenko twice, Cyborg, Rousey, Holm, Tate, de Randamie, and Pena revenge give Nunes the deepest women’s ledger.'],
    ...
    'Valentina Shevchenko': {
      ...
      longevity: ['Valentina’s case is long technical excellence across bantamweight and flyweight.'],
  ...
  function voicePhrase(f,key,other){
    const bank = FIGHTER_VOICE[f.fighter]?.[key];
    if(!Array.isArray(bank) || !bank.length) return null;
    return pick(bank, pairKey(f, other || f, `voice-${key}`));
  }
  ...
  function detailFor(f,other,role,hasWinsLine){
    ...
    const voiceKey = voiceKeyForCategory(top);
    const voice = voiceKey ? voicePhrase(f, voiceKey, other) : null;
    ...
    const byCategory = { championship: p.championship || p.titleSummary, opponentQuality: opponentFallback, primeDominance: p.peak || p.primeSummary, longevity: p.longevity || p.resume, penalty: p.weakness || p.shortCase };
  ...
  function render(){
    ...
    const frame = matchupFrame(a,b);
    const paragraphs = [frame,argumentParagraph(loser, winner, 'loser', type),argumentParagraph(winner, loser, 'winner', type),swingParagraph(winner, loser, type, typeInfo.ledger),distinctionParagraph(winner, loser)].filter(Boolean);
    rendering = true;
  ...
  function install(){
    ...
    document.querySelectorAll('.tab').forEach(tab => { if(!tab.dataset.compareNarrativeTabListener){ tab.dataset.compareNarrativeListener = 'true'; tab.addEventListener('click', () => setTimeout(render, 0)); } });
  }
