(function(){
  ...
  function waitForModelReady(){
    if(window.UFC_SCORING_PIPELINE?.status === 'ready' || document.documentElement.getAttribute('data-scoring-pipeline') === 'ready'){
  ...
  function resumeProgressIfNeeded(){
    ...
    try { shouldResume = sessionStorage.getItem(PROGRESS_KEY) === '1'; }
    catch(_error){}
  ...
  function installButton(){
    ...
    button.addEventListener('click',() => {
      ...
      window.setTimeout(() => window.location.replace(url.toString()),320);
    });
