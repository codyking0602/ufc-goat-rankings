...
// Keeps the existing Compare-tab flow while reading canonical live app data.
(function(){
  'use strict';

  const VERSION = 'octagon-verdict-compare-launcher-20260711b-canonical-cards';
  ...
  function overrides(){
    if(window.DISPLAY_OVERRIDES) return window.DISPLAY_OVERRIDES;
    try { if(typeof DISPLAY_OVERRIDES !== 'undefined') return DISPLAY_OVERRIDES; } catch(error) {}
    return {};
  }
  ...
  function fmt(value,digits=1){ const n=finite(value); return n === undefined ? '—' : n.toFixed(digits); }
  function clamp(value,min,max){ return Math.max(min,Math.min(max,value)); }
  function overrideFor(f){ return overrides()[f.fighter] || {}; }
  function packetFor(f){ return packets()[f.fighter] || {}; }
  function snapshotRows(f){ return overrideFor(f).snapshot || packetFor(f).display?.snapshot || []; }
  function snapshotValue(f,labels){
    const wanted=labels.map(label=>String(label).toLowerCase());
    const row=snapshotRows(f).find(item=>Array.isArray(item)&&wanted.includes(String(item[0]||'').toLowerCase()));
  ...
  function statBridge(f){
    return {
      ...(packetFor(f).profileStats || {}),
      ...(overrideFor(f).packetProfileStats || {}),
      ...(overrideFor(f).snapshotStats || {})
    };
  }
  function qualityAudit(f){
    return f.opponentQualityLiveAudit
      || f.opponentQualityShadowAudit
      || window.UFC_OPPONENT_QUALITY_SHADOW_AUDIT?.summaryFor?.(f.fighter)
      || window.UFC_OPPONENT_QUALITY_LIVE?.report?.find?.(row=>row.fighter===f.fighter)
      || {};
  }
  function boardType(f){ return (data().women || []).some(row=>row.fighter===f.fighter) ? 'women' : 'men'; }
  function fallbackOvr(f){
    const anchors=boardType(f)==='women'
      ? {floorScore:25.78,ceilingScore:80.79}
      : {floorScore:18.68,ceilingScore:101.92};
    const score=finite(f.totalScore) ?? anchors.floorScore;
    const normalized=clamp((score-anchors.floorScore)/(anchors.ceilingScore-anchors.floorScore),0,1);
    return clamp(Math.round(82+Math.pow(normalized,0.85)*17),82,99);
  }
  function overallOvrFor(f){
    const direct=finite(f.overallOvr);
    if(direct !== undefined) return direct;
    const engine=window.UFC_FINAL_SCORE_ENGINE?.overallOvrFor;
    if(typeof engine==='function') return engine(f);
    try { if(typeof overallOvr==='function') return overallOvr(f); } catch(error) {}
    return fallbackOvr(f);
  }
  function rankFor(f){ return firstDefined(f.rank,overrideFor(f).allTimeRank,'—'); }
  function divisionFor(f){
    return firstDefined(overrideFor(f).divisionLabel,[f.primaryDivision,f.secondaryDivision].filter(Boolean).join(' / '),f.division,'—');
  }
  function ufcRecord(f){ return firstDefined(f.ufcRecord,statBridge(f).ufcRecord,snapshotValue(f,['UFC Record']),'—'); }
  function titleFightWins(f){
    const bridge=statBridge(f);
    const direct=firstDefined(f.titleFightWins,bridge.titleFightWins);
    if(direct!==undefined) return direct;
    const snapshot=snapshotValue(f,['UFC Title-Fight Wins','Title-Fight Wins','Title Wins']);
    const parsed=firstNumber(snapshot);
    if(parsed!==undefined) return parsed;
    const noteMatch=String(f.title?.notes||'').match(/Total title fight wins = ([0-9.]+)/i);
    return noteMatch ? Number(noteMatch[1]) : '—';
  }
  function qualityWins(f){
    const audit=qualityAudit(f);
    const bridge=statBridge(f);
    const elite=firstDefined(audit.elitePlusWins,f.elitePlusWins,bridge.elitePlusWins);
    const topFive=firstDefined(audit.topFivePlusWins,f.topFivePlusWins,bridge.topFivePlusWins);
    if(elite!==undefined || topFive!==undefined) return `${elite ?? '—'} / ${topFive ?? '—'}`;
    const legacy=firstDefined(bridge.eliteWins,f.eliteWins,firstNumber(snapshotValue(f,['Elite Wins','Quality Wins'])));
    return legacy===undefined ? '—' : String(legacy);
  }
  function activeEliteYears(f){
    return firstDefined(statBridge(f).activeEliteYears,f.activeEliteYears,firstNumber(snapshotValue(f,['Active Elite Years','Elite Years'])));
  }
  function oneLine(f){
    const o=overrideFor(f);
    return firstDefined(o.resumeTag,o.oneLiner,o.compareProfile?.shortCase,f.resumeTag,'Current ranking case');
  }
  ...
  function fighterCard(f,sideLabel){
    return `<article class="ov-fighter-card">
      ...
      <h3>${f.fighter}</h3>
      <div class="ov-card-meta"><b>#${rankFor(f)}</b><span>${overallOvrFor(f)} OVR</span><span>${divisionFor(f)}</span></div>
      <p>${oneLine(f)}</p>
      <div class="ov-stat-grid">
        ...
        <div><strong>${qualityWins(f)}</strong><small>Elite+ / Top-5+ wins</small></div>
        <div><strong>${fmt(activeEliteYears(f),1)}</strong><small>Elite years</small></div>
      </div>
    </article>`;
  }
  function render(){
    ...
    node.innerHTML=`
      ...
      <section class="ov-card-grid">${fighterCard(a,'Fighter A')}${fighterCard(b,'Fighter B')}</section>`;
