(function(){
  ...
  const VERSION='picks-season-loop-20260718c-phase-2c-speed';
  ...
  const state={loading:null,data:null,identity:null,social:null,bound:false,groupObserver:null,groupTarget:null,recapObserver:null,recapTarget:null,standingsObserver:null,standingsTarget:null,timer:0};
  ...
  function installStyles(){
    ...
    style.textContent=`
      #picksSeasonHub,#picksProfileShell,.picks-home-profile,.picks-home-current,#picksEventView>.picks-view-heading{display:none!important}
  ...
  async function loadData(force=false){
    ...
    state.loading=(async()=>{
      ...
      state.data={identity,group,me,rank:canonicalRank(group,me),records,main,upcoming,events,eventMap,year:currentYear()};
  ...
  function canonicalStats(data){const me=data.me||{};const made=num(me.picks_made);const correct=num(me.correct);const losses=Math.max(0,made-correct);const accuracy=Number.isFinite(Number(me.accuracy))?num(me.accuracy):(made?Math.round(correct/made*100):0);return{made,correct,losses,accuracy,points:num(me.points),wins:num(me.event_wins),rank:data.rank,best:bestFinish(data.records)};}
  function summaryMarkup(data){const stats=canonicalStats(data);return`<section id="picksSeasonSummary" class="picks-season-summary"><header class="picks-season-summary-head"><div><span>${data.year} PICKS SEASON</span><strong>Your season</strong><small>${stats.points} points · ${stats.wins} event win${stats.wins===1?'':'s'} · Best finish ${stats.best}</small></div><div class="picks-season-summary-rank"><b>${esc(stats.rank)}</b><em>GOAT26 RANK</em></div></header><div class="picks-season-summary-grid"><div><span>SEASON RECORD</span><strong>${stats.correct}-${stats.losses}</strong><small>${stats.made} graded picks</small></div><div><span>ACCURACY</span><strong>${stats.accuracy}%</strong><small>${stats.correct}/${stats.made} correct</small></div><div><span>MAIN EVENTS</span><strong>${data.main.correct}-${data.main.losses}</strong><small>${data.main.made} graded</small></div><div><span>UNDERDOG HITS</span><strong>${data.records.reduce((sum,row)=>sum+row.underdogHits,0)}</strong><small>${data.records.reduce((sum,row)=>sum+row.lockHits,0)} lock bonus hit${data.records.reduce((sum,row)=>sum+row.lockHits,0)===1?'':'s'}</small></div></div></section>`;}
  ...
  function patchCanonicalHome(data){
    document.getElementById('picksSeasonHub')?.remove();document.querySelector('#picksHomeCompact .picks-home-profile')?.remove();document.querySelector('#picksHomeCompact .picks-home-current')?.remove();document.getElementById('picksProfileShell')?.setAttribute('hidden','');
  ...
  function currentEvent(){const id=document.getElementById('picksEventSelect')?.value||new URL(location.href).searchParams.get('event');return state.data?.eventMap?.get(id)||state.data?.upcoming||null;}
  function patchEventStandings(){const target=document.getElementById('picksStandings');if(!target)return;const started=gradedFights(currentEvent()).length>0;if(started)return;target.querySelectorAll('.picks-standing-rank').forEach(rank=>{if(rank.textContent!=='—')rank.textContent='—';});}
  ...
  function ensureProfileAction(){
    const overlay=document.querySelector('.profile-activity-overlay');if(!overlay||!state.data)return;
    const stats=canonicalStats(state.data);
    const summary=[...overlay.querySelectorAll('.profile-activity-summary .profile-activity-stat')].find(node=>node.querySelector('span')?.textContent.trim()==='PICKS RECORD');
    if(summary){setText(summary.querySelector('strong'),`${stats.correct}-${stats.losses}`);setText(summary.querySelector('small'),`${stats.accuracy}% correct`);}
    const card=[...overlay.querySelectorAll('.profile-activity-card')].find(node=>node.querySelector('.profile-activity-card-head strong')?.textContent.trim()==='Season Activity');if(!card)return;
    setText(card.querySelector('.profile-activity-card-head small'),`${stats.rank} in GOAT26 · ${state.data.main.correct}-${state.data.main.losses} main events`);
    [...card.querySelectorAll('.profile-activity-metric')].forEach(metric=>{
      const label=metric.querySelector('span')?.textContent.trim();
      if(label==='POINTS'){setText(metric.querySelector('strong'),stats.points);setText(metric.querySelector('small'),'Total');}
      if(label==='ACCURACY'){setText(metric.querySelector('strong'),`${stats.accuracy}%`);setText(metric.querySelector('small'),`${stats.correct}/${stats.made}`);}
      if(label==='SEASON RANK'){setText(metric.querySelector('strong'),stats.rank);setText(metric.querySelector('small'),`Best ${stats.best}`);}
    });
    let button=card.querySelector('[data-open-picks-season]');if(!button){button=document.createElement('button');button.type='button';button.className='profile-picks-season-action';button.dataset.openPicksSeason='true';button.textContent='OPEN PICKS SEASON';card.appendChild(button);}
  }
  function renderAll(){installStyles();watchTargets();if(!state.data)return;patchCanonicalHome(state.data);ensureSummary();ensureReminder();patchEventStandings();ensurePersonalRecap();ensureProfileAction();updateCountdown();}
  function updateCountdown(){const node=document.querySelector('#picksSeasonReminder>div:first-child small');if(!state.data?.upcoming||!node)return;const lock=nextLock(state.data.upcoming);node.textContent=lock?`Locks ${formatDate(lock.lockAt,true)} · ${remainingLabel(lock.lockAt)} remaining`:`Card starts ${formatDate(eventDate(state.data.upcoming),true)`};}
  ...
  function bind(){
    ...
    document.addEventListener('click',event=>{if(event.target.closest?.('.app-profile-chip')){[0,80,220].forEach(delay=>setTimeout(()=>state.data?ensureProfileAction():loadData(),delay));return;}if(event.target.closest?.('[data-picks-season-calendar]')){addCalendar(state.data?.upcoming);return;}if(event.target.closest?.('[data-open-picks-season]')){openSeason();return;}});
  ...
  function watchTargets(){
    ...
    if(standings&&standings!==state.standingsTarget){state.standingsObserver?.disconnect();state.standingsTarget=standings;state.standingsObserver=new MutationObserver(()=>requestAnimationFrame(patchEventStandings));state.standingsObserver.observe(standings,{childList:true,subtree:true});}
  }
  function start(){installStyles();bind();document.getElementById('picksSeasonHub')?.remove();watchTargets();[60,240,700].forEach(delay=>setTimeout(watchTargets,delay));if(document.getElementById('picks')?.classList.contains('active-view'))loadData();}
