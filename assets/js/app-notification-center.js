(function(){
  ...
  const VERSION='app-notification-center-20260718a-phase-3';
  const CANONICAL_CODE='GOAT26';
  const TOKEN_KEY=`ufc-picks:group:${CANONICAL_CODE}`;
  const state={settings:null,identity:null,busy:false,observer:null,renderTimer:0,status:'',statusKind:''};
  ...
  function urlBase64ToUint8Array(value){const clean=text(value),padding='='.repeat((4-clean.length%4)%4),base64=(clean+padding).replace(/-/g,'+').replace(/_/g,'/'),raw=atob(base64);return Uint8Array.from([...raw].map(character=>character.charCodeAt(0)));}

  async function registration(create=false){
    if(!('serviceWorker' in navigator))return null;
    let current=await navigator.serviceWorker.getRegistration().catch(()=>null);
    if(!current&&create){
      current=await navigator.serviceWorker.register('sw.js?v=octagon-hq-sw-20260718b-fast-refresh',{scope:'./'});
      await navigator.serviceWorker.ready;
    }else if(current&&create){
      try{await current.update();}catch(_error){}
    }
    return current;
  }
  ...
  async function enableDevice(permissionPromise){
    ...
    state.busy=true;setStatus('Finishing notification setup…');
    try{
      ...
      const who=await identity(),rpc=client(),token=tokenFor(who);
      if(!settings||!rpc||!token)throw new Error('Reconnect your Octagon HQ profile and try again.');
      const publicKey=text(settings.vapid_public_key);
      if(!publicKey)throw new Error('Push notification keys are not ready.');
      const worker=await registration(true);
  ...
  async function disableDevice(){
    if(state.busy)return false;
    state.busy=true;setStatus('Turning off notifications on this device…');
    try{
      ...
      if(subscription&&rpc&&token){
        ...
        if(error)throw error;
        if(!data?.ok)throw new Error(data?.error||'The device subscription could not be removed.');
  ...
  async function updatePreference(key,value){
    ...
    const prefs={...state.settings.preferences,[key]:Boolean(value)};
    state.settings.preferences=prefs;scheduleRender();
    try{
      const who=await identity(),rpc=client(),token=tokenFor(who);
      const {data,error}=await rpc.rpc('app_notification_update_preferences',{
        p_member_token:token,
        p_direct_challenges:Boolean(prefs.direct_challenges),
        p_picks_reminders:Boolean(prefs.picks_reminders),
        p_war_room_messages:Boolean(prefs.war_room_messages)
      });
      if(error)throw error;
      if(!data?.ok)throw new Error(data?.error||'Notification preferences could not save.');
      state.settings.preferences=data.preferences||prefs;
  ...
  async function sendTest(){
    ...
    state.busy=true;setStatus('Queuing a test notification…');
    try{
      ...
      if(error)throw error;
      if(!data?.ok)throw new Error(data?.error||'The test notification could not be queued.');
  ...
  function cardMarkup(){
    ...
    if(!settings)return '<article class="profile-activity-card wide app-notification-card" data-app-notification-center><header class="profile-activity-card-head"><div><span>NOTIFICATIONS</span><strong>Smart alerts</strong></div><small>Loading…</small></header><div class="profile-activity-empty">Connecting notification settings to your Octagon HQ profile.</div></article>';
    const enabled=Boolean(settings.current_device_enabled);
    const prefs=settings.preferences||{};
    return `<article class="profile-activity-card wide app-notification-card" data-app-notification-center data-signature="${enabled}|${Boolean(prefs.direct_challenges)}|${Boolean(prefs.picks_reminders)}|${Boolean(prefs.war_room_messages)}|${state.busy}|${text(state.status)}">
      ...
      <div class="app-notification-options">
        ${optionMarkup('direct_challenges','DIRECT CHALLENGES','When another profile sends you an exact game board.',prefs.direct_challenges)}
        ${optionMarkup('picks_reminders','PICKS CLOSING SOON','Only when your active event still has missing picks.',prefs.picks_reminders)}
        ${optionMarkup('war_room_messages','WAR ROOM MESSAGES','New posts, plus replies in discussions you joined.',prefs.war_room_messages)}
      </div>
  ...
  function render(){
    const grid=document.querySelector('.profile-activity-body .profile-activity-grid');
    if(!grid)return false;
    const existing=grid.querySelector('[data-app-notification-center]');
    const holder=document.createElement('div');holder.innerHTML=cardMarkup();const next=holder.firstElementChild;
    if(existing?.dataset.signature&&existing.dataset.signature===next?.dataset.signature)return true;
  ...
  function start(){
    installStyles();
    state.observer=new MutationObserver(()=>{if(document.querySelector('.profile-activity-body .profile-activity-grid')){scheduleRender();if(!state.settings)void loadSettings();}});
    ...
    window.addEventListener('octagon-hq:notification-device-change',()=>void loadSettings());
