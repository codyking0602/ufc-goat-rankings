(function(){
  ...
  function installStyles(){
    ...
    style.textContent=`
      [data-octagon-alerts],.octagon-alerts-button,.octagon-notification-status{display:none!important}
      .app-notification-card{border:1px solid rgba(249,115,22,.46);border-radius:18px;background:#101827;color:#f8fafc;padding:15px}.app-profile-notification-section{margin:0 0 18px}.profile-activity-card.app-notification-card{border-color:rgba(249,115,22,.46)!important}
      .app-notification-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:12px}.app-notification-head span,.app-notification-head strong,.app-notification-head small{display:block}.app-notification-head span{color:#fb923c;font:950 9px/1 system-ui;letter-spacing:.12em}.app-notification-head strong{margin-top:6px;font:950 18px/1 system-ui}.app-notification-head small{color:#94a3b8;font:750 10px/1.3 system-ui;text-align:right}
      .app-notification-device{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px;border:1px solid #33445f;border-radius:15px;background:#0b1220}.app-notification-device strong,.app-notification-device small{display:block}.app-notification-device strong{font:900 12px/1.2 system-ui}.app-notification-device small{margin-top:5px;color:#94a3b8;font:700 9px/1.35 system-ui}.app-notification-device-actions{display:flex;gap:7px;flex:0 0 auto}.app-notification-device button{min-height:38px;border:1px solid #475569;border-radius:11px;background:#172033;color:#fff;padding:0 11px;cursor:pointer;font:950 9px/1 system-ui}.app-notification-device button.primary{border-color:#f97316;background:#f97316;color:#111827}.app-notification-device button:disabled{opacity:.48;cursor:wait}
      .app-notification-options{display:grid;gap:7px;margin-top:10px}.app-notification-option{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:12px;padding:10px 12px;border:1px solid #2f3e56;border-radius:13px;background:#0b1220}.app-notification-option strong,.app-notification-option small{display:block}.app-notification-option strong{font:900 11px/1.2 system-ui}.app-notification-option small{margin-top:4px;color:#94a3b8;font:700 9px/1.3 system-ui}.app-notification-switch{position:relative;width:42px;height:24px}.app-notification-switch input{position:absolute;opacity:0;pointer-events:none}.app-notification-switch span{position:absolute;inset:0;border:1px solid #475569;border-radius:999px;background:#172033;transition:.15s ease}.app-notification-switch span:after{content:"";position:absolute;top:3px;left:3px;width:16px;height:16px;border-radius:50%;background:#94a3b8;transition:.15s ease}.app-notification-switch input:checked+span{border-color:#f97316;background:rgba(249,115,22,.26)}.app-notification-switch input:checked+span:after{left:21px;background:#fb923c}.app-notification-switch input:focus-visible+span{outline:2px solid #f97316;outline-offset:2px}.app-notification-status{min-height:18px;margin-top:9px;color:#fdba74;font:800 9px/1.35 system-ui}.app-notification-status.error{color:#fca5a5}.app-notification-loading{padding:13px;border:1px dashed #475569;border-radius:13px;color:#94a3b8;text-align:center;font:750 10px/1.4 system-ui}
      @media(max-width:600px){.app-notification-device{align-items:flex-start;flex-direction:column}.app-notification-device-actions{display:grid;grid-template-columns:1fr 1fr;width:100%}.app-notification-device button{width:100%}.app-notification-head{display:grid}.app-notification-head small{text-align:left}}
    `;
    document.head.appendChild(style);
  }

  function purgeLegacyWarRoomControls(root=document){
    root.querySelectorAll?.('[data-octagon-alerts],.octagon-alerts-button,.octagon-notification-status').forEach(node=>node.remove());
  }
  ...
  async function registerStoredSubscription(subscription,who){
    ...
    if(!data?.ok)throw new Error(data?.error||'This device could not be registered.');
    return true;
  }
  ...
  async function loadSettings(force=false){
    ...
    try{
      ...
      if(error)throw error;
      if(!data?.ok)throw new Error(data?.error||'Notification settings could not load.');
    ...
    finally{state.loading=false;scheduleRender();}
  }
  ...
  async function enableDevice(permissionPromise){
    ...
    try{
      ...
      setStatus('Notifications are on for this device.');await loadSettings(true);
      window.dispatchEvent(new CustomEvent('octagon-hq:notification-device-change',{detail:{enabled:true}}));
      return true;
  ...
  async function disableDevice(){
    ...
    try{
      ...
      setStatus('Notifications are off for this device.');await loadSettings(true);
      window.dispatchEvent(new CustomEvent('octagon-hq:notification-device-change',{detail:{enabled:false}}));
      return true;
  ...
  async function updatePreference(key,value){
    ...
    }catch(error){setStatus(text(error?.message)||'Notification preferences could not save.','error');await loadSettings(true);}
  }
  ...
  function markup(surface){
    ...
    const className=surface==='activity'?'profile-activity-card wide app-notification-card':'app-profile-notification-section app-notification-card';
    if(!settings){
      const signature=`${surface}|loading|${state.loading}`;
      return `<article class="${className}" data-app-notification-center="${surface}" data-signature="${signature}"><header class="app-notification-head"><div><span>NOTIFICATIONS</span><strong>Smart alerts</strong></div><small>Profile settings</small></header><div class="app-notification-loading">${state.loading?'Loading notification settings…':'Open your profile to connect notification settings.'}</div></article>`;
    }
    const enabled=Boolean(settings.current_device_enabled),prefs=settings.preferences||{};
    const signature=[surface,enabled,Boolean(prefs.direct_challenges),Boolean(prefs.picks_reminders),Boolean(prefs.war_room_messages),state.busy,text(state.status),state.statusKind].join('|');
    return `<article class="${className}" data-app-notification-center="${surface}" data-signature="${signature}">
      <header class="app-notification-head"><div><span>NOTIFICATIONS</span><strong>Smart alerts</strong></div><small>${enabled?'On for this device':'Off on this device'}</small></header>
      <div class="app-notification-device"><div><strong>${enabled?'This device can receive alerts':'Enable alerts on this device'}</strong><small>Challenges, missing Picks, and War Room messages only.</small></div><div class="app-notification-device-actions">${enabled?`<button type="button" data-app-notification-test ${state.busy?'disabled':''}>SEND TEST</button><button type="button" data-app-notification-disable ${state.busy?'disabled':''}>TURN OFF</button>`:`<button type="button" class="primary" data-app-notification-enable ${state.busy?'disabled':''}>ENABLE</button>`}</div></div>
      <div class="app-notification-options">${optionMarkup('direct_challenges','DIRECT CHALLENGES','When another profile sends you an exact game board.',prefs.direct_challenges)}${optionMarkup('picks_reminders','PICKS CLOSING SOON','Only when the active event still has missing picks.',prefs.picks_reminders)}${optionMarkup('war_room_messages','WAR ROOM MESSAGES','New posts and replies in discussions you joined.',prefs.war_room_messages)}</div>
      <div class="app-notification-status${state.statusKind==='error'?' error':''}" role="status">${text(state.status)}</div>
    </article>`;
  }

  function bindCard(card){
    card.querySelector('[data-app-notification-enable]')?.addEventListener('click',enableFromClick);
    card.querySelector('[data-app-notification-disable]')?.addEventListener('click',()=>void disableDevice());
    card.querySelector('[data-app-notification-test]')?.addEventListener('click',()=>void sendTest());
    card.querySelectorAll('[data-app-notification-pref]').forEach(input=>input.addEventListener('change',()=>void updatePreference(input.dataset.appNotificationPref,input.checked)));
  }

  function renderInto(host,surface,before=null){
    if(!host)return false;
    const existing=host.querySelector(`:scope > [data-app-notification-center="${surface}"]`);
    const holder=document.createElement('div');holder.innerHTML=markup(surface);const next=holder.firstElementChild;
    if(existing?.dataset.signature===next?.dataset.signature)return true;
    if(existing)existing.replaceWith(next);
    else if(before&&before.parentElement===host)host.insertBefore(next,before);
  ...
  function render(){
    purgeLegacyWarRoomControls();
    const profileBody=document.querySelector('.app-profile-panel .app-profile-body');
    const photoSection=profileBody?.querySelector('.app-profile-photo-section');
    renderInto(profileBody,'profile',photoSection);
    const activityGrid=document.querySelector('.profile-activity-body .profile-activity-grid');
    renderInto(activityGrid,'activity');
    return Boolean(profileBody||activityGrid);
  }

  function scheduleRender(){window.clearTimeout(state.renderTimer);state.renderTimer=window.setTimeout(render,40);}

  function start(){
    if(state.started)return;state.started=true;
    installStyles();purgeLegacyWarRoomControls();void registration(true).catch(()=>undefined);
    state.observer=new MutationObserver(records=>{
      let relevant=false;
      records.forEach(record=>record.addedNodes.forEach(node=>{
        if(node.nodeType!==1)return;
        purgeLegacyWarRoomControls(node);
        if(node.matches?.('.app-profile-panel,.profile-activity-panel,[data-octagon-board]')||node.querySelector?.('.app-profile-panel,.profile-activity-panel,[data-octagon-board]'))relevant=true;
      }));
      if(relevant){scheduleRender();if(!state.settings&&!state.loading)void loadSettings();}
    });
    state.observer.observe(document.body,{childList:true,subtree:true});
    ['ufc-play-profile-ready','ufc-app-profile-updated'].forEach(name=>window.addEventListener(name,event=>{state.identity=event.detail?.identity||event.detail||state.identity;void loadSettings(true);}));
    window.addEventListener('octagon-hq:notification-device-change',()=>void loadSettings(true));
    window.setTimeout(()=>void loadSettings(),250);
  }

  window.UFC_APP_NOTIFICATIONS={version:VERSION,loadSettings,render,enableFromClick,enableDevice,disableDevice,sendTest,updatePreference,get settings(){return state.settings;},get enabled(){return Boolean(state.settings?.current_device_enabled);}};
