...
try{
  ...
  const report=await page.evaluate(()=>{
    ...
    return{
      version:'quality-anchor-discovery-20260710a',generatedAt:new Date().toISOString(),readOnly:true,
      currentBenchmarkCredit:round(live.benchmarkCredit,2),
      ...
      futureLeaderShock:[12,14,16].filter(x=>x>num(live.benchmarkCredit)).map(newBenchmark=>({newBenchmark,legacyMultiplier:round(num(live.benchmarkCredit)/newBenchmark,4),exampleCurrentScore20Becomes:round(20*num(live.benchmarkCredit)/newBenchmark,2)})),
      rows
    };
  });

  const f=n=>Number(n||0).toFixed(2);
  let md=`# Quality Wins Anchor Discovery\n\nGenerated: ${report.generatedAt}\n\nRead-only: yes. No live score, total, rank, OVR, ledger, or formula was changed.\n\n## Current conversion\n\n- Current benchmark: ${f(report.currentBenchmarkCredit)} diminished Quality Wins credits.\n- Current formula: ${report.currentFormula||'Unavailable'}\n- Roster: ${report.rosterCount} fighters.\n\n## Distribution\n\n| Input | Min | P10 | P25 | Median | P75 | P90 | Max | Mean | Std dev |\n|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n`;
  ...
  if(report.futureLeaderShock.length){md+=`\n## Current-leader normalization risk\n\nThe current Quality Wins score is normalized to the current roster leader. A future leader can lower every existing score.\n\n| Future leader credit | Legacy multiplier | Example current 20/30 becomes |\n|---:|---:|---:|\n`;for(const x of report.futureLeaderShock)md+=`| ${f(x.newBenchmark)} | ${f(x.legacyMultiplier)} | ${f(x.exampleCurrentScore20Becomes)} |\n`;}
  ...
  console.log('QUALITY_ANCHOR_DISCOVERY='+JSON.stringify({fighters:report.rosterCount,benchmark:report.currentBenchmarkCredit,creditStats:report.stats.diminishedCredit}));
}finally{
  await browser.close();
}
