...
const missingPrimeRoundRows=report.fighters.flatMap(row=>(row.stats.missingRoundRows||[]).map(fight=>({fighter:row.fighter,...fight})));
const missingEliteRoundRows=report.fighters.flatMap(row=>(row.stats.eliteLevelValidation?.missingRoundRows||[]).map(fight=>({fighter:row.fighter,...fight})));
if(missingPrimeRoundRows.length)console.log('MISSING_PRIME_ROUND_ROWS',JSON.stringify(missingPrimeRoundRows,null,2));
if(missingEliteRoundRows.length)console.log('MISSING_ELITE_ROUND_ROWS',JSON.stringify(missingEliteRoundRows,null,2));
assert.equal(report.missingPrimeRoundRowCount,0,`Every scored prime fight must have audited round data. Missing: ${JSON.stringify(missingPrimeRoundRows)}`);
assert.equal(report.missingEliteStageRoundRowCount,0,`Every elite-stage prime fight must have audited round data. Missing: ${JSON.stringify(missingEliteRoundRows)}`);
