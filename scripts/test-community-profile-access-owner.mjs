...
try{
  ...
  assert.equal(initial.group,'community-owner-token');
  assert.equal(initial.room,'community-owner-token');
  assert.equal(initial.active,'GOAT26');
  assert.equal(initial.display,'Cody');
  assert.equal(initial.writes.some(row=>/community-profiles\.js/i.test(row.stack)),false,'Community profiles must not persist shared Picks access.');
  assert.equal(initial.history.some(row=>/community-profiles\.js/i.test(row.stack)),false,'Community profiles must not own Picks route compatibility writes.');
  ...
  assert.equal(await page.evaluate(()=>window.__PIN_REFRESH_COUNT__),1,'One profile update must produce one product-owned Picks compatibility refresh.');
  assert.equal(await page.evaluate(()=>window.__COMMUNITY_ACCESS_WRITES__.some(row=>/community-profiles\.js/i.test(row.stack))),false);
  ...
  assert.equal(picks.group,'GOAT26','Product compatibility must retain the Picks group route handoff.');
  assert.equal(picks.refreshes,1,'One Picks view change must produce one product-owned compatibility refresh.');
  assert.equal(picks.history.some(row=>/community-profiles\.js/i.test(row.stack)),false);
  assert.equal(picks.cardSuppressed,true,'The shared-profile duplicate Picks card suppression must remain active.');

  const snapshot=await page.evaluate(()=>window.UFC_COMMUNITY_PROFILES.load(true));
  assert.equal(snapshot?.ok,true,'Community profile loading must remain functional after access ownership removal.');
