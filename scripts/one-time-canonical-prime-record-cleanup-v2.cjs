...
function canonicalizeRankingData(rows) {
  ...
  const profiles = sandbox.window.RANKING_DATA?.fighters || [];
  const map = new Map(rows.map(row => [row.fighter, row]));
  if (profiles.length !== 62 || map.size !== 62) {
    throw new Error(`Expected 62 profiles and records; got ${profiles.length}/${map.size}`);
  }
  for (const profile of profiles) {
    const row = map.get(profile.fighter);
    if (!row || !RECORD_RE.test(String(row.record || ''))) {
      throw new Error(`Invalid resolved Prime Record for ${profile.fighter}: ${row?.record}`);
    }
  }
  ...
  for (const profile of profiles) {
    ...
    if (count !== 1) {
      throw new Error(`Expected one canonical profile row for ${profile.fighter}; found ${count}`);
    }
    ...
    section = section.replace(
      needle,
      `${needle}      "primeRecord": ${JSON.stringify(row.record)},\n${context}`
    );
  }
...
function cleanDisplayOverrides() {
  ...
  const source = stripPrimeRecordStrings(read(file));
...
function updateRuntimeLoader() {
  ...
  source = source.split('\n').filter(line =>
    !line.includes('ranking-data-additions.js') &&
    !line.includes('fighter-packet-schema.js') &&
    !line.includes('assets/data/fighter-packets.js') &&
