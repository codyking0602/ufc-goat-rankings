...
try{
  ...
  const report=await page.evaluate(()=>{
    ...
    const LIVE_BENCHMARK=round(live.benchmarkCredit,2);
    const SOURCE_BENCHMARK=round(live.sourceBenchmarkCredit??Math.max(...live.report.map(x=>num(x.diminishedCredit)),1),2);
    ...
    const scenarios={
      retiredDynamic:{label:'Retired current-leader normalization',anchors:[[0,0],[SOURCE_BENCHMARK,30]],score:x=>clamp(x/SOURCE_BENCHMARK*30,0,30),rosterStable:false},
      currentFixed:{label:'Current locked 14.10-credit linear anchors',anchors:[[0,0],[2.35,5],[4.70,10],[7.05,15],[9.40,20],[11.75,25],[14.10,30]],score:x=>clamp(x/LOCKED_BENCHMARK*30,0,30),rosterStable:true},
    ...
    const out={version:'quality-anchor-review-20260710b-live-fixed',generatedAt:new Date().toISOString(),readOnly:true,liveBenchmarkCredit:LIVE_BENCHMARK,sourceBenchmarkCredit:SOURCE_BENCHMARK,lockedBenchmarkCredit:LOCKED_BENCHMARK,liveBenchmarkMode:live.benchmarkMode||null,liveFutureRosterStable:Boolean(live.futureRosterStable),liveFormula:live.formula||null,inputFormula:win.UFC_OPPONENT_QUALITY_LEDGERS?.formula||null,creditScale:win.UFC_OPPONENT_QUALITY_LEDGERS?.creditScale||null,diminishingReturns:win.UFC_OPPONENT_QUALITY_LEDGERS?.diminishingReturns||null,categoryMeanings:{0:'No meaningful UFC quality-win credit',5:'Limited quality-win résumé',10:'Solid ranked-win résumé',15:'Established elite-win résumé',20:'Great quality-win résumé',25:'All-time quality-win résumé',30:'Historical benchmark'},scenarios:{}};
  ...
  md+=`## Roster-growth stability\n\n| Future leader credit | Retired dynamic multiplier | Example current 20/30 would become | Current fixed 20/30 remains |\n|---:|---:|---:|---:|\n`;
  for(const x of report.futureLeaderShock)md+=`| ${f(x.newBenchmark)} | ${f(x.retiredDynamicMultiplier)} | ${f(x.exampleCurrentScore20WouldBecome)} | ${f(x.currentFixedScore20Remains)} |\n`;
  ...
  console.log('QUALITY_ANCHOR_REVIEW='+JSON.stringify({fighters:62,liveBenchmark:report.liveBenchmarkCredit,sourceBenchmark:report.sourceBenchmarkCredit,lockedBenchmark:report.lockedBenchmarkCredit,mode:report.liveBenchmarkMode,stable:report.liveFutureRosterStable,scenarios:Object.keys(report.scenarios)}));
}finally{
  await browser.close();
}
