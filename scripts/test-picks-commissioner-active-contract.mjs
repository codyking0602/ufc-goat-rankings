...
assert.match(start[1],/if\(picksActive\(\)\) refresh\(\);/,'Direct Picks startup must still request commissioner state.');
assert.match(start[1],/octagon-hq:view-change[\s\S]*destination==='picks'[\s\S]*refresh\(\)/,'Entering Picks must request commissioner state.');
...
console.log(JSON.stringify({passed:true,responsibility:'Picks commissioner snapshot activation',removed:['Home startup RPC','hidden Picks mutation refresh','hidden 45-second polling'],preserved:['direct Picks startup','route entry','late card mount','active freshness polling']},null,2));
