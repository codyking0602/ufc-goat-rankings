...
1. approved first-run, cached, uncached, retry, and explicit-action behavior still works; and
2. duplicated ownership or repeated work is removed, delegated, or coalesced.
...
5. record current in-flight/coalescing behavior and legitimate recovery paths.
...
8. verify the exact tested head is current, or prove later `main` movement is unrelated;
9. merge with exact expected head/base locking when appropriate.

## Static ownership contract

The focused contract must enforce the real responsibility boundary, not a generic marker.

As applicable, reject:
...
- identity-dependent RPC work before cached/published identity exists;
- duplicate startup handoffs;
- a second unconditional readiness refresh;
- removal of a required in-flight/coalescing guard;
- reintroduction of a removed route, profile, notification, Picks, War Room, or access owner.

The contract must preserve explicit user-action boundaries such as `UFC_PLAY_PROFILE.require()` when sign-in is genuinely needed.

## Focused mobile-browser proof

Use a mobile viewport and instrument the exact owner boundary.
...
- competing readiness, route, visibility, online, realtime, timer, polling, or direct calls: one in-flight request;
...
A proof failure is evidence to investigate. Do not dismiss it as flaky until the exact failure is understood. If the proof expectation is wrong, correct only the proof and rerun the complete gate without changing runtime.
...
- product behavior whose correctness depends on the installed-app container rather than browser semantics.
...
- exact tested runtime/test blobs are unchanged under a required generated child commit.
...
- Focused static and browser proofs pass.
...
- Multiple follow-up patches are required to restore surrounding surfaces.
