...
This file records the canonical owner of each startup responsibility. Detailed history remains in the Phase 1–4 audits and progress ledgers.
...
| Picks season summary/events/social/room loading | `assets/js/picks-season-loop.js` | Passive identity consumer with a coalesced season request set |
...
Startup Architecture Gate and the dedicated mobile suites protect:
...
Phase 5 may simplify the production script manifest and load order only after proving a wiring edge obsolete. It must preserve every ownership contract above.