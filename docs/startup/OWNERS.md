...
| Calculated production scoring bootstrap | `assets/js/production-ranking-bootstrap.js` | Canonical calculated-production launcher | **Next isolated Phase 1 owner.** Explicit apply/retry lifecycle must be designed and proven before duplicate execution can be blocked; do not add a naive top-level marker |
...
| Sharing and incoming share routing | `assets/js/share-deep-links.js` | Canonical share/deep-link owner | Global duplicate-file guard merged and physical-iPhone verified in PR #121; first-run API publication, native share and clipboard fallbacks, profile and War Room observers, delayed passes, ranking-ready and `popstate` retries, all five supported incoming routes, and surrounding route handoffs preserved |
